...
# Functions for finding full zeta function (depreciated)
...
def main():
    p = Integer(input("Prime [3]: ") or 3)
    q = Integer(input("Order of Finite Field [3]: ") or 3)
    f = log(q, p)
    ...
    r = Integer(input("Number of Terms [4]: ") or 4)
    coeffs, powers = [], []

    for i in range(r):
        coeffs.append(Integer(input("Coefficient [1]: ") or 1))
        powers.append(Integer(input("Power [4]: ") or 4))

    r -= 1

    ambient_space = input("Weighted Projective (WP) or Affine Solutions (A) [WP] : ")
    m = Integer(input("Maximum Computation Degree [60]: ") or 60)
    wp = True
