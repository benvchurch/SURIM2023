...
def multiple_newton_polygon_plot(min_prime=2, max_prime=2,
    ...
    exponent_sets = list(map(list, sorted(itertools.product(*[list(range(min_exponent, max_exponent + 1))]*len(coeffs)))))
    primes = [Integer(p) for p in [p for p in primesfrom2to(int(max_prime)+1) if p >= min_prime]]
                ...
                try:
                    ...
                    if profile is True:
                        try:
                            ...
                            line = '{},{},{},{},None,{},{},{},{},{},{}\n'.format(*list(map(str, [prime, '_'.join(map(str, exponent_set)), smoothness, supersingularity, round(raw_speed, time_precision),
                    ...
                    if profile is True:
                        try:
                            stats = profiler.getstats()[-1]
                            line = '{},{},N/A,N/A,TimeoutException,N/A,{},{},{},{},{}\n'.format(*list(map(str, [prime, '_'.join(map(str, exponent_set)), stats.calls, stats.callcount, stats.reccallcount, stats.inlinetime, stats.totaltime])))
                        except IndexError:
                            line = '{},{},N/A,N/A,TimeoutException,N/A,N/A,N/A,N/A,N/A,N/A\n'.format(*list(map(str, [prime, '_'.join(map(str, exponent_set))])))
                    else:
                        line = '{},{},N/A,N/A,TimeoutException,N/A,N/A,N/A,N/A,N/A,N/A\n'.format(*list(map(str, [prime, '_'.join(map(str, exponent_set))])))
                    print(line)
                    outfile.write(line)
                    ...
                    if profile is True:
                        try:
                            stats = profiler.getstats()[-1]
                            line = '{},{},N/A,N/A,{},N/A,{},{},{},{},{}\n'.format(*list(map(str, [prime, '_'.join(map(str, exponent_set)), e.message, stats.calls, stats.callcount, stats.reccallcount, stats.inlinetime, stats.totaltime])))
                        except IndexError:
                            line = '{},{},N/A,N/A,{},N/A,N/A,N/A,N/A,N/A,N/A\n'.format(*list(map(str, [prime, '_'.join(map(str, exponent_set)), e.message])))
                    else:
                        line = '{},{},N/A,N/A,{},N/A,N/A,N/A,N/A,N/A,N/A\n'.format(*list(map(str, [prime, '_'.join(map(str, exponent_set)), e.message])))
                    print(line)
                    outfile.write(line)
...
def search_over_primes(min_prime=2, max_prime=100, min_exponent=3,
    ...
    with open('data/{}/surface_search_over_primes.{}.total.csv'.format('affine' if affine else 'weighted_projective', start), 'a') as total_outfile:
        ...
        for exponent_set, primes in sorted(iter(parameters.items()), key = lambda p: list(map(int, p[0].split('_')))):
...
def execute():
    affine = bool(int(input('Affine (1) or Weighted Projective (0) [1]: ').strip().lower() or 1))
    min_prime = Integer(input('Minimum Prime [2]: ') or 2)
    max_prime = Integer(input('Maximum Prime [100]: ') or 100)
    min_exponent = Integer(input('Minimum Exponent [3]: ') or 3)
    max_exponent = Integer(input('Maximum Exponent [25]: ') or 25)
    timeout = int(input('Timeout in Seconds [30]: ') or 30)
    time_precision = int(input('Precision of Profile Reporting [6]: ') or 6)
    profile = int(input('Profile (1) or Not (0) [0]: ') or 0)
